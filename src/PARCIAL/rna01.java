...
//        double test7[]={1,1,0}; usored(test7);
    ...
    public double prueba(double[][] pruebas, double[][] salida){
        ...
        for(int i=0;i<pruebas.length;i++){
            ...
            double s = usored(prubs, salida[i]);
            if(Math.abs(s-salida[i][0])/Math.max(s, salida[i][0]) <0.5){
                acc++;
            }
        }
        acc /= pruebas.length;
    ...
    public double usored(double[] datatest, double[] salida){
